...
/**
 * 수험번호 자리수 — 학생 출결번호를 그대로 쓴다.
 *
 * 시험마다 다르게 정할 수 있게 두면 같은 학생의 번호가 시험마다 다른 자리수로
 * 찍힌다. 그러면 성적표를 학생과 이어 붙이지 못하고, 학생도 매번 몇 자리를
 * 칠해야 하는지 헷갈린다. 학원 전체가 5자리를 쓰므로 여기서 고정한다.
 *
 * 예전에 다른 자리수로 만들어 둔 시험은 각자의 값을 그대로 쓴다 — 답안지가
 * 이미 나갔을 수 있어 뒤늦게 바꾸면 그 답안지를 못 읽는다.
 */
export const FIXED_ID_DIGITS = 5;

/**
 * 학원 대표 전화 — 알림톡 미리보기의 '문의' 줄에 쓴다.
 *
 * 알림톡 본문은 심사받은 템플릿에 박혀 있어 우리가 바꿀 수 없다. 화면
 * 미리보기는 그 문구를 그대로 비추는 거울이므로, 여기 값과 솔라피에 등록한
 * 템플릿의 번호가 **같아야** 미리보기가 제 구실을 한다.
 */
export const ACADEMY_PHONE = "031-794-3306";
